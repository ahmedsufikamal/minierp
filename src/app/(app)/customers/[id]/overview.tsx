...
import { CheckCircle2, Circle, Clock, Mail, Phone, Users, MessageSquare, Plus } from "lucide-react";
               ...
               {customer.activities.map((activity) => (
                 <div key={activity.id} className="relative z-10">
                   <div className="absolute -left-[21px] rounded-full bg-white border border-slate-200 p-1">
                      ...
                      {activity.type === 'NOTE' && <MessageSquare className="h-3 w-3 text-slate-500" />}
                   </div>
                   
                   <div className="text-sm">
                     <span className="font-medium text-slate-900">{activity.type}</span>
                     <span className="text-slate-500 mx-1">•</span>
                     <span className="text-slate-500">{format(new Date(activity.createdAt), "MMM d, h:mm a")}</span>
                   </div>
          ...
          <CardContent className="p-4 space-y-4">
             ...
             {customer.tasks.map(task => (
               <div key={task.id} className="flex items-start gap-3 group">
                 ...
                 <div className="text-sm">
                   <div className={`text-slate-900 ${task.status === 'DONE' ? 'line-through text-slate-400' : ''}`}>
                     {task.title}
                   </div>
                   {task.dueDate && (
        ...
        <Card>
          ...
          <CardContent className="p-4 text-sm space-y-3">
             <div className="grid grid-cols-3 gap-2">
                ...
                <div className="col-span-2 text-slate-900 font-medium">{format(new Date(customer.createdAt), "MMM d, yyyy")}</div>
             </div>
              <div className="grid grid-cols-3 gap-2">
