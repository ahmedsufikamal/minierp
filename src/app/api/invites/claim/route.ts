import { headers } from "next/headers";
import { getIdentityProvider, requireAuth } from "@/modules/iam";
import { IamError } from "@/modules/iam/domain/errors";
import { assertRateLimit } from "@/modules/iam/infrastructure/rate-limit";
import { err, ok, parseBody } from "@/modules/iam/interface/http";
import { assertSameOrigin } from "@/modules/iam/interface/origin";
import { claimInviteSchema } from "@/modules/iam/interface/schemas";

function safeInt(value: string | undefined, fallback: number, min = 1, max = 10_000): number {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.min(max, Math.max(min, Math.floor(parsed)));
}

function isInviteSignupBridgeEnabled(): boolean {
  const explicit = process.env.IAM_INVITE_SIGNUP_BRIDGE_ENABLED;
  if (explicit === "1") return true;
  if (explicit === "0") return false;
  return true;
}

export async function POST(request: Request) {
  try {
    assertSameOrigin(request);
    if (!isInviteSignupBridgeEnabled()) {
      throw new IamError("FORBIDDEN", "Invite claim is disabled");
    }

    const principal = await requireAuth();
    const body = await parseBody(request, claimInviteSchema);

    const h = await headers();
    const forwarded = h.get("x-forwarded-for");
    const ip = forwarded?.split(",")[0]?.trim() || h.get("x-real-ip") || "unknown";
    await assertRateLimit({
      scope: "invite_claim",
      key: `${principal.userId}:${ip}`,
      maxAttempts: safeInt(process.env.IAM_INVITE_CLAIM_RATE_LIMIT_MAX_ATTEMPTS, 20, 1, 200),
      windowSeconds: safeInt(process.env.IAM_INVITE_CLAIM_RATE_LIMIT_WINDOW_SECONDS, 60, 10, 3600),
    });

    await getIdentityProvider().claimInvite({
      token: body.token,
      userId: principal.userId,
      userEmail: principal.email,
    });

    return ok({ accepted: true });
  } catch (error) {
    return err(error);
  }
}
