...
const COLUMNS: OpportunityStage[] = ["NEW", "QUALIFICATION", "PROPOSAL", "NEGOTIATION", "WON"];
...
export function KanbanBoard({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeItem && activeItem.stage !== newStage) {
      // Optimistic update
      setItems((prev) =>
        prev.map((item) => (item.id === activeItem.id ? { ...item, stage: newStage } : item)),
      );
  ...
  const getItemsByStage = (stage: OpportunityStage) => items.filter((item) => item.stage === stage);
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      ...
      <DragOverlay dropAnimation={dropAnimation}>
        {activeId ? <KanbanCard item={items.find((i) => i.id === activeId)!} isOverlay /> : null}
      </DragOverlay>
    </DndContext>
  );
}
...
function KanbanColumn({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "p-3 rounded-t-xl border-b flex items-center justify-between",
          colorClass,
